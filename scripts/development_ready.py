"""Select a verified main revision for development; never authorize pull-request source."""
...
def main():
    ...
    for deployment in api(
        f"repos/{repo}/deployments?sha={sha}&environment=development&per_page=100"
    ):
        statuses = api(
    ...
    print(
        f"Revision {sha}: {'ready for development' if ready else 'not eligible or already deployed'}"
    )
